...
def fit(model,
    ...
    for epoch in pbar:
        ...
        if best_result_dict is None or best_i_roc_prompt < result_dict['i_roc_prompt']:
            best_i_roc_prompt = result_dict['i_roc_prompt']
            best_epoch_prompt = epoch
        
        if best_result_dict is None or best_i_roc < result_dict['i_roc']:
            save_check_point(model, check_path)
            best_result_dict = result_dict
            best_epoch = epoch
            best_i_roc = result_dict['i_roc']

        pbar.set_postfix(Best_I_AUROC=best_i_roc, Best_I_AUROC_Prompt=best_i_roc_prompt, Best_Ep=best_epoch, Best_Ep_Prompt=best_epoch_prompt, I_AUROC_Mem=result_dict['i_roc_vad'], I_AUROC=result_dict['i_roc'], I_AUROC_Prompt=result_dict['i_roc_prompt'])
