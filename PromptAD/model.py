...
class PromptLearner(nn.Module):
    def __init__(self, n_ctx, n_pro, n_ctx_ab, n_pro_ab, classname, clip_model, pre):
        ...
        abnormal_ctx_vectors = torch.empty(n_pro_ab, n_ctx_ab, ctx_dim, dtype=dtype) # n_ctx_ab이 왜 100이 되는 거지? -> length를 100으로 설정했나?
        # 애초에 이 abnormal_ctx_vectors가 어떻게 이렇게 설정이 된 건지 잘 모르겠다
        ...
        abnormal_prompts_handle = [normal_prompt_prefix + " " + state.format(classname) + "." for state in state_anomaly1 for _ in range(n_pro)] # 이 부분이 manual anomaly prompt 부분
        abnormal_prompts_learned = [normal_prompt_prefix + " " + abnormal_prompt_prefix + " " + classname + "." for _ in range(n_pro_ab) for _ in range(n_pro)] # lap

        # abnormal_prompts = abnormal_prompts_learned + abnormal_prompts_handle

        tokenized_normal_prompts = CLIPAD.tokenize(normal_prompts)
        tokenized_abnormal_prompts_handle = torch.cat([CLIPAD.tokenize(p) for p in abnormal_prompts_handle]) # 42, 77
        tokenized_abnormal_prompts_learned = torch.cat([CLIPAD.tokenize(p) for p in abnormal_prompts_learned]) # 12, 77
    ...
    def forward(self):
        ...
        abnormal_prefix_learned = self.abnormal_token_prefix_learned # 12,1,640
        abnormal_suffix_learned = self.abnormal_token_suffix_learned # 12,0,640
        abnormal_ctx = self.abnormal_ctx # 4,100,640
        n_pro_ad, n_ctx_ad, dim_ad = abnormal_ctx.shape
        normal_ctx2 = normal_ctx.unsqueeze(0).expand(self.n_pro_ab, -1, -1, -1).reshape(-1, n_ctx, dim) # 12, 4 ,640
        abnormal_ctx = abnormal_ctx.unsqueeze(0).expand(self.n_pro, -1, -1, -1).reshape(-1, n_ctx_ad, dim_ad) # 12, 100, 640
